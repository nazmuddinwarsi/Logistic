...
Public Class frmConsignee
    Dim cnn As New DataConn.Conn
    Private Sub frmConsignee_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmConsignee_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub BindGrid()
        ...
        sSQL = ""
        strSQL = "SELECT AccountID, AccountName, CPerson, Address, City, Mobile, GSTinNo, AadharNo, CASE WHEN RateBy='P' THEN 'PACKAGE' ELSE 'WEIGHT' END AS RateBy, ProductRate, Remarks FROM LOG_tblAccount WHERE tType='CNSE'"
        ...
        Grid.Columns(8).HeaderText = "Rate By"
        Grid.Columns(9).HeaderText = "Rate"
        Grid.Columns(10).HeaderText = "Remarks"
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        '--------------------- CHECK DUPLICATE
        If lblID.Text = "" Then
            strSQL = "Select AccountID from LOG_tblAccount Where AccountName=" & cnn.SQLquote(UCase(txtCompany.Text)) & " AND tType='CNSE'"
        Else
            strSQL = "Select AccountID from LOG_tblAccount Where AccountName=" & cnn.SQLquote(UCase(txtCompany.Text)) & " AND tType='CNSE' AND AccountID <>" & Val(lblID.Text)
        End If
        ...
        If lblID.Text <> "" Then
            ...
            & ", AadharNo=" & cnn.SQLquote(UCase(txtAadharNo.Text)) & ", ProductRate=" & Val(txtProdRate.Text) & ", EditedBy = " & Val(MDI.lblUserLoginID.Text) _
            & ", Remarks=" & cnn.SQLquote(UCase(txtRemarks.Text)) & ", EditedDate='" & cnn.GetDate(Date.Today()) & "', RateBy='" & iRateBy & "' WHERE AccountID=" & Val(lblID.Text)
        Else
            strSQL = "INSERT INTO LOG_tblAccount (tType, AccountName, CPerson, Address, City, State, Mobile, GSTinNo, AadharNo, CreatedBy, CreatedDate, ProductRate, Remarks, RateBy) VALUES ('CNSE'," & cnn.SQLquote(UCase(txtCompany.Text)) _
            ...
            & "," & cnn.SQLquote(UCase(txtMobileNo.Text)) & "," & cnn.SQLquote(UCase(txtGST.Text)) & "," & cnn.SQLquote(UCase(txtAadharNo.Text)) & "," & Val(MDI.lblUserLoginID.Text) & ",'" & cnn.GetDate(Date.Today()) & "'" _
            & "," & Val(txtProdRate.Text) & "," & cnn.SQLquote(UCase(txtRemarks.Text)) & ",'" & iRateBy & "')"
        End If
    ...
    Private Sub ClearText()
        txtCompany.Text = "" : txtName.Text = "" : txtAddress.Text = "" : txtProdRate.Text = "" : txtCity.Text = "" : txtState.Text = "" : txtMobileNo.Text = "" : txtGST.Text = ""
        lblID.Text = "" : txtRemarks.Text = "" : txtCompany.Focus()
    End Sub
    ...
    Private Sub DeleteRecord()
        ...
        Dim strSQL As String
        '--------------------------------------------------------------
        strSQL = "Select InvoiceID from LOG_tblInvoice Where ConsigneeID =" & Val(lblID.Text)
        If cnn.CheckDuplicate(strSQL) = True Then
    ...
    Private Sub frmConsignee_Resize(sender As Object, e As EventArgs) Handles Me.Resize
        FormRes()
    End Sub
    Private Sub FormRes()
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Dim reportDesc As String = "Consignee List"
        strReportName = Application.StartupPath & "\Reports\AccountList2.rpt"
    ...
    Private Sub DisplayReport(ByVal CrxReport As CrystalDecisions.CrystalReports.Engine.ReportDocument, ByVal reportDesc As String)
        ...
        strSelection = "{rptInvoice.tType} = 'CNSE'"
        If txtSName.Text <> "" Then
        ...
        CrxReport.Refresh()
        CrxReport.DataDefinition.FormulaFields("lblCaption").Text = "'Consignee List'"
