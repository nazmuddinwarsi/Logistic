...
Public Class frmCity
    ...
    Private Sub frmCity_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub BindGrid()
        ...
        sSQL = ""
        strSQL = "SELECT ID, fldName FROM LOG_tblMaster WHERE tType='CITY' ORDER BY fldName"
        ...
        Grid.Columns(1).HeaderText = "Destination Name"
    End Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        '--------------------- CHECK DUPLICATE
        If lblID.Text = "" Then
            strSQL = "Select ID from LOG_tblMaster Where fldName=" & cnn.SQLquote(UCase(txtName.Text)) & " AND tType='CITY'"
        Else
            strSQL = "Select ID from LOG_tblMaster Where fldName=" & cnn.SQLquote(UCase(txtName.Text)) & " AND tType='CITY' AND ID <>" & Val(lblID.Text)
        End If
        ...
        '--------------------- CHECK DUPLICATE
        If lblID.Text <> "" Then
            strSQL = "UPDATE LOG_tblMaster SET fldName=" & cnn.SQLquote(UCase(txtName.Text)) _
            & ", EditedBy = " & Val(MDI.lblUserLoginID.Text) & ", EditedDate='" & cnn.GetDate(Date.Today()) & "' WHERE ID=" & Val(lblID.Text)
        Else
            strSQL = "INSERT INTO LOG_tblMaster (fldName, tType, CreatedBy, CreatedDate) VALUES (" & cnn.SQLquote(UCase(txtName.Text)) & ",'CITY'," & Val(MDI.lblUserLoginID.Text) & ",'" & cnn.GetDate(Date.Today()) & "')"
        End If
    ...
    Private Sub DeleteRecord()
        ...
        Dim strSQL As String
        '--------------------------------------------------------------
        strSQL = "Select InvoiceID from LOG_tblInvoice Where DestinationID =" & Val(lblID.Text)
        If cnn.CheckDuplicate(strSQL) = True Then
    ...
    Private Sub frmCity_Resize(sender As Object, e As EventArgs) Handles Me.Resize
        FormRes()
    End Sub
    Private Sub FormRes()
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Dim reportDesc As String = "City List"
    ...
    Private Sub DisplayReport(ByVal CrxReport As CrystalDecisions.CrystalReports.Engine.ReportDocument, ByVal reportDesc As String)
        ...
        strSelection = "{LOG_tblMaster.tType} = 'CITY'"

        '===== Generate report preview =====================
        Cursor.Current = Cursors.WaitCursor
        CrxReport.Refresh()
        CrxReport.DataDefinition.FormulaFields("lblCaption").Text = "'City List'"
